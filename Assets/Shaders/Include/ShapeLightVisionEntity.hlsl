...
half4 CombinedShapeLightShared(half4 color, half4 mask, half2 lightingUV, half2 worldUV)
{
    ...
    half4 visionMask = SAMPLE_TEXTURE2D(_ShapeLightTexture0, sampler_ShapeLightTexture0, lightingUV);
    
    if (any(_ShapeLightMaskFilter0))
    {
        float4 processedMask = (1 - _ShapeLightInvertedFilter0) * mask + _ShapeLightInvertedFilter0 * (1 - mask);
        visionMask *= dot(processedMask, _ShapeLightMaskFilter0);
    }
    ...
    half4 shapeLight3Modulate = shapeLight3 * _ShapeLightBlendFactors1.x;
    half4 shapeLight3Additive = shapeLight4 * _ShapeLightBlendFactors1.y;
#else
    ...
    finalOutput = color;
#else
    half4 finalModulate = shapeLight3Modulate + shapeLight2Modulate + shapeLight1Modulate;
    half4 finalAdditive = shapeLight3Additive + shapeLight2Additive + shapeLight1Additive;
